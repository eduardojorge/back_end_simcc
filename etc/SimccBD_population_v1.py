...
def insert_researcher_frequency_db(teste, article, offset):
    ...
    reg = sgbdSQL.consultar_db(
        "SELECT   r.id from researcher r, bibliographic_production b where r.id =b.researcher_id "
        + filter
        + " OFFSET "
        + str(offset)
...
def insert_researcher_frequency_bigrama_db(teste, article):
    ...
    for i, infos in df_bd.iterrows():
        ...
        print(infos.id)
        reg = sgbdSQL.consultar_db(
            "SELECT  term_1,term_2  FROM research_dictionary_bigram WHERE term_1 <> term_2 ORDER BY frequency desc fetch FIRST 200 rows only"
        )
        ...
        for i, infos in df_bd.iterrows():
            if infos.term_1 != infos.term_2:
                ...
                reg = sgbdSQL.consultar_db(
                    "SELECT b.id as id from bibliographic_production AS b"
                    + " WHERE "
                    + "  AND researcher_id='"
                    + researcher_id
                    + "'"
                    + filter
                    + " AND (       translate(unaccent(LOWER(b.title)),':','') ::tsvector@@ '"
                    + unidecode.unidecode(term)
                    + "'::tsquery)=true"
                )
...
def create_area_ditionary_db():
    ...
    reg = sgbdSQL.consultar_db(
        "SELECT a.name as term_1,b.name as term_2   from area_expertise a,  area_expertise b "
    ...
    values('%s','%s');""" % (
            item.lower(),
            list_word_area[item].get("frequencia"),
        )
        ...
        if not (
            ...
            if word_previous != "":
                ...
                if key.lower() in lista_word_area_bigram:
                    frequencia_bigram = (
                        lista_word_area_bigram[key.lower()].get("frequencia")
                    ) + 1
                    term_1 = lista_word_area_bigram[key.lower()].get("term_1")
                    term_2 = lista_word_area_bigram[key.lower()].get("term_2")

                    lista_word_area_bigram[key.lower()] = {
                        "term_1": term_1,
                        "term_2": term_2,
                        "frequencia": frequencia_bigram,
                    }
                else:
                    lista_word_area_bigram[key.lower()] = {
                        "term_1": word_previous.lower(),
                        "term_2": word.lower(),
                        "frequencia": 1,
                    }
...
def create_researcher_dictionary_db(test, article):
    ...
    sql = (
        "SELECT   r.id from researcher r,bibliographic_production b where r.id=researcher_id "
        + filter
    )
    ...
    values('%s','%s','%s');""" % (
            item.lower(),
            list_word[item].get("frequencia"),
            list_word[item].get("type"),
        )
...
def create_researcher_title_dictionary_db(researcher_id, article):
    ...
    reg = sgbdSQL.consultar_db(
        "SELECT  distinct title,b.type,year from bibliographic_production AS b"
        + " WHERE "
        +
        # is_new= true and
        "researcher_id='"
...
for i in range(4928):
    if (i % 100) == 0:
        print(i)
        t = threading.Thread(
            target=insert_researcher_frequency_db,
            args=(teste, article, i),
            name="t" + str(i),
        )
        t.start()
