...
def graduate_program_student_researcher_csv_db():
    ...
    df_bd = pd.DataFrame(
        reg, columns=["lattes_id", "graduate_program_id", "year", "type_"]
    )
...
def cimatec_article_qualis_distinct_csv_db():
    reg = sgbdSQL.consultar_db(
        " SELECT distinct title,bar.qualis,bar.jcr,b.year as year,gp.graduate_program_id as graduate_program_id,gpr.year as year_pos "
        + "   "
