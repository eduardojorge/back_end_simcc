...
def new_researcher_production_db(researcher_id):
    ...
    sql="SELECT  count(title) as qtd,b.type as tipo from bibliographic_production AS b  where researcher_id='%s' GROUP BY tipo ORDER  BY Tipo desc" % (researcher_id)
...
logging.basicConfig(filename = "logfile_Production.log",
                    filemode = "w",
                    format = Log_Format, 
                    level = logging.DEBUG)
