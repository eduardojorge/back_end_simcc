...
def production_db(researcher_id):
    reg = sgbdSQL.consultar_db(
        "SELECT  distinct title,b.type,year from bibliographic_production AS b, bibliographic_production_author AS ba"
...
def bibliographic_production_total_db():
    reg = sgbdSQL.consultar_db(
        "SELECT COUNT(DISTINCT title) as qtd FROM bibliographic_production "
    )
    #'where id=\'35e6c140-7fbb-4298-b301-c5348725c467\''+
    #' OR id=\'c0ae713e-57b9-4dc3-b4f0-65e0b2b72ecf\' ')
...
def list_originals_words_initials_term_db(initials):
    ...
    reg = sgbdSQL.consultar_db(
        "SELECT  originals_words "
        + " FROM researcher_term rt "
        + " WHERE "
        + "  LOWER(unaccent(term)) LIKE '"
        + initials
        + "%'"
    )
...
def list_area_expertise_term_db(term):
    ...
    reg = sgbdSQL.consultar_db(
        "SELECT (a.name)  AS area_expertise_, a.id as id  from area_expertise a "
        + " WHERE "
        + "  unaccent(LOWER(a.name)) LIKE '"
        + term.lower()
        + "%' "
        + " ORDER BY a.name "
    )
...
def list_researcher_area_expertise_term_db(term):
    ...
    reg = sgbdSQL.consultar_db(
        "SELECT unaccent((a.name || ' | ' || b.name))  AS area_expertise , r.name "
        ...
        + " AND a.parent_id IS NOT NULL "
        + " AND (       translate(unaccent(LOWER((a.name ||' ' || b.name))),':','') ::tsvector@@ '"
        + term
        + "'::tsquery)=true"
    )
...
def lists_bibliographic_production_article_db(
    ...
    if distinct == "0":
        ...
        sql = """
            ...
            FROM
                institution i
                JOIN PUBLIC.bibliographic_production b ON i.id = b.institution_id
                JOIN bibliographic_production_article a ON a.bibliographic_production_id = b.id
