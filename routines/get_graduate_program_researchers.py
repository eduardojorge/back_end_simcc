...
driver.find_element(By.XPATH, path).click() # apertar botão dos coockies
with open('graduate_program_researchers.csv', 'w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    instituicoes = [ #lista do código das instituições de ensino superior da bahia
        "28007018",
        "28008014",
        "28011015",
        "28002016",
    ...
    for instituicao in instituicoes:
        ...
        time.sleep(2)
        select_programa = Select(driver.find_element(By.NAME, "form:j_idt33:j_idt113"))
        ...
        while (index < last_index):
            ...
            driver.execute_script("arguments[0].scrollIntoView();", consultar_button)
            ...
            while bool:
                try:
                    ...
                    break
                table_rows_doscentes = table_body_doscentes.find_elements(By.TAG_NAME, "tr")
                ...
                button_proxima = WebDriverWait(driver, 10).until(
                    EC.visibility_of_element_located((By.ID, "form:j_idt80:botaoProxPagina"))
                )
                button_proxima.click()
                if (button_proxima.tag_name == 'span'): #verifica se não há mais paginas
                    bool = False
                time.sleep(2)
            index = index + 1
        #limpa campo do seletor de instituição:
        WebDriverWait(driver, 50000).until(EC.presence_of_element_located((By.NAME, "form:j_idt33:inst:input"))).clear()
driver.quit()