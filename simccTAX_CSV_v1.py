...
for i,infos in df.iterrows():
    ...
    for combinacao_ in t:
       ...
       for term_ in t_:
          term = unidecode.unidecode(combinacao_.lower())+";"+str(unidecode.unidecode(term_.lower()))
          word = unidecode.unidecode(term_.lower()).strip()

        
          df1_article =  list_researchers_article_abstract_tax_db(str(infos[CODIGO]),term,word,"ARTICLE")
